...
`i18n/en.json` holds every user-visible string of the configurator, nested by feature and addressed
by a dotted key:
...
Outputs: `configurator/src/shared/ui-string-keys.ts` (the `UiStringKey` union and the parameter
types) and `configurator/src/shared/ui-strings-en.ts` (the table). Code reads them through `t()` in
`configurator/src/shared/ui-text.ts`, which main and renderer share. The two outputs are split
because every call site imports the keys while only the resolver imports the table, and the table
carries an explicit `Readonly<Record<string, string>>` rather than `as const` so no project builds a
literal type over the whole catalog. A plural message carries a `$plural` object of CLDR categories
(`one`, `other`, …) selected at runtime by `Intl.PluralRules` — the marker is explicit because the
catalog also holds maps keyed by enum members, and a reset cause is legitimately named `other`.
Placeholders are `{name}`. A `$note` field is data for the translator, not a comment. See
[ADR 0031](docs/adr/0031-centralized-ui-strings.md).
...
- **File size**: keep source files at roughly 300 lines or fewer. A change that pushes a file past
  ...
  (`*_generated.*`, the generated `.ts` mirrors, `google-fonts-catalog.json`, `ui-string-keys.ts`,
  `ui-strings-<locale>.ts`) are exempt.
