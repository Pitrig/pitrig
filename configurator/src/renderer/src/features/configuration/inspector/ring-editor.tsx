...
export function RingFields({ widget, owner, update }: { widget: RingWidget; owner: 'arc' | 'indicator'; update: (mutation: (next: RingWidget) => void) => void }): React.JSX.Element {
  ...
  return (
    <>
      ...
      />
      <SelectField label={t('inspector.ringEditor.centreOn')} hint={t('inspector.hints.ring.centering')} value={widget.centering ?? 'circle'} options={RING_CENTERING_VALUES} modified={authored(widget.centering, 'circle')} onReset={() => update((next) => { delete next.centering })} onChange={(value) => update((next) => { next.centering = value })} />
