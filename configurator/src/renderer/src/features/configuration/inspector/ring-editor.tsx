...
export function RingFields({ widget, owner, update }: { widget: RingWidget; owner: 'arc' | 'indicator'; update: (mutation: (next: RingWidget) => void) => void }): React.JSX.Element {
  ...
  return (
    <>
      <SliderField label={t('inspector.ringEditor.start')} hint={t('inspector.hints.ring.start')} suffix="degrees" value={widget.start_angle_deg ?? 135} min={angleBounds.min ?? 0} max={angleBounds.max ?? 359} modified={authored(widget.start_angle_deg, 135)} onReset={() => update((next) => { delete next.start_angle_deg })} onChange={(value) => update((next) => { next.start_angle_deg = value })} />
      <SliderField label={t('inspector.ringEditor.sweep')} hint={t('inspector.hints.ring.sweep')} suffix="degrees" value={widget.sweep_deg ?? 270} min={sweepBounds.min ?? 1} max={sweepBounds.max ?? 360} modified={authored(widget.sweep_deg, 270)} onReset={() => update((next) => { delete next.sweep_deg })} onChange={(value) => update((next) => { next.sweep_deg = value })} />
      ...
      <SliderField
        label={t('inspector.indicatorEditor.radius')}
        hint={t('inspector.hints.ring.radius')}
        suffix="px"
        value={radius !== 0 ? radius : fitted}
        min={0}
        max={radiusBounds.max ?? 2048}
        softMax={Math.min(radiusBounds.max ?? 2048, Math.max(256, 4 * fitted))}
        caption={radius === 0 ? t('inspector.ringEditor.followingTheBoxFittedPx', { fitted: fitted }) : t('inspector.ringEditor.theBoxOnItsOwn', { fitted: fitted })}
        modified={radius !== 0}
