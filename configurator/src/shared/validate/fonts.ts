...
export function findFontError(configuration: ApplicationConfiguration): string | undefined {
  ...
  for (const font of fonts) {
    if (
      ...
      return t('validation.fonts.everyDashboardFontMustExplicitly')
    }
  }
  const families = new Set(fonts.map((font) => font?.family))
  if (families.size > MAXIMUM_FONT_FAMILIES) {
    return t('validation.fonts.configurationReferencesMoreThanMaximum', { mAXIMUM_FONT_FAMILIES: MAXIMUM_FONT_FAMILIES })
  }
  return undefined
}
