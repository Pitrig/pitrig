...
collector, the overlays and the `@SC:DIAG` responder live outside the product
tree entirely.
...
`configuration_control` source file, gated by `#if SIMCORE_DEBUG` but compiled
...
`CONFIG_SIMCORE_DEBUG`. Requirements stay unconditional because ESP-IDF resolves them in
...
Production code keeps only one-line hooks behind `SIMCORE_DEBUG`.
...
`@SC:DIAG` is a hook rather than a debug-owned method: `configuration_control`
...
`SIMCORE_DEBUG` and `SIMCORE_LAYOUT_DEBUG` are normalized to `1`/`0` like their
...
- The production T-Display image lost 176 bytes of flash and 1 840 bytes of
  internal `.bss`, and contains no SimCore debug symbol.
...
- A production build answers `@SC:DIAG` with `@SC:ERR:unsupported`. Diagnosing a
  board in the field means flashing a debug build over `@SC:FW:`.
