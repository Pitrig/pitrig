...
- Правила умовного стилю на текстовому віджеті: до чотирьох правил над одним
  полем, яке віджет може й не показувати; колір значення, фон, колір рамки,
  приховування та блимання.
...
Колір за порогом, приховування за умовою і блимання вже є: текстовий віджет
стежить за одним полем телеметрії (незалежним від того, що показує) і має до
чотирьох правил; перше, що спрацювало, задає колір значення, фон, колір рамки,
видимість і період блимання, решта лишається як в авторському стилі
([ADR 0017](adr/0017-conditional-widget-styling.md)). Немає: прозорості як
властивості, градієнтів, обертання, кривих анімації та тригерів. Delta Time
лишається зі своєю власною моделлю тонів.
...
Пункти 3 і 5 змінюють `configuration/configuration_schema.json` і фірмвару, тож
кожен потребує ADR (або оновлення наявного) до реалізації.
