...
std::size_t encode(rmt_encoder_t* const base,
  ...
  if (encoder.state == 0) {
    written += encoder.bytes->encode(encoder.bytes, channel, payload, size,
                                     &session);
  ...
  if (encoder.state == 1) {
    written += encoder.copy->encode(encoder.copy, channel, &encoder.reset,
                                    sizeof(encoder.reset), &session);
    if ((session & RMT_ENCODING_COMPLETE) != 0) {
      encoder.state = 0;
...
esp_err_t encoder_open(ChainEncoder& encoder,
                       const std::uint32_t resolution_hz) {
  const rmt_bytes_encoder_config_t bytes{
      .bit0 = {{kZeroHighTicks, 1, kZeroLowTicks, 0}},
      .bit1 = {{kOneHighTicks, 1, kOneLowTicks, 0}},
      .flags = {.msb_first = 1},
  };
  if (const esp_err_t error = rmt_new_bytes_encoder(&bytes, &encoder.bytes);
      error != ESP_OK) {
    return error;
  }
  const rmt_copy_encoder_config_t copy{};
  if (const esp_err_t error = rmt_new_copy_encoder(&copy, &encoder.copy);
      error != ESP_OK) {
    encoder_close(encoder);
    return error;
  }
  const auto half = static_cast<std::uint16_t>(
      resolution_hz / 1'000'000 * kResetMicroseconds / 2);
