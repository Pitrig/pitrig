...
void Surface::set(const std::size_t index, const led::Color color) const {
  ...
  const led::Color ink =
      brightness_ == 255 ? color : color.scaled(brightness_);
  if (!mirrored_) {
    output_->set(physical(index), ink);
    return;
  }
  const std::size_t half = size();
  const std::size_t low = half - 1 - index;
  const std::size_t high = count_ - half + index;
  output_->set(physical(low), ink);
  output_->set(physical(high), ink);
}

std::optional<Surface> surface_of(led::Output& output, const std::size_t lamps,
                                  const configuration::LedEffect& effect) {
  if (effect.from >= lamps) {
    return std::nullopt;
  }
  const auto available = static_cast<std::uint16_t>(lamps - effect.from);
  const std::uint16_t count =
      effect.count == 0 ? available : std::min(effect.count, available);
  if (count == 0) {
    return std::nullopt;
  }
  return Surface{output,          effect.from,     count,
                 effect.inverted, effect.mirrored, effect.brightness};
}
