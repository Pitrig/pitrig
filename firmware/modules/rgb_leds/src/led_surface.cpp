...
void Surface::put(const std::size_t offset, const led::Color color) const {
  ...
  const std::size_t lamp = led::matrix_lamp(*matrix_, area_.x + column, area_.y + row);
...
bool Area::holds(const std::uint16_t column, const std::uint16_t row) const {
  ...
  const std::size_t pixel = static_cast<std::size_t>(y + row) * stride + (x + column);
...
Area area_of(const led::Matrix& matrix, const configuration::LedEffect& effect) {
  ...
  if (down == 1) {
    ...
    const std::uint16_t count = effect.count == 0 ? available : std::min(effect.count, available);
...
Surface surface_of(led::Output& output, const led::Matrix& matrix, const Area& area,
...
void fill_area(led::Output& output, const led::Matrix& matrix, const Area& area,
  ...
  for (std::uint16_t row = 0; row < area.height; ++row) {
    for (std::uint16_t column = 0; column < area.width; ++column) {
      ...
      const std::size_t lamp = led::matrix_lamp(matrix, area.x + column, area.y + row);
      if (lamp != led::kOffPanel) {
