...
[[nodiscard]] std::int32_t text_width(const lv_font_t* const font, const char* const text) {
  lv_point_t size{};
...
[[nodiscard]] Rect caption_rect(const CaptionLayout& layout, const std::int32_t width,
  ...
  return {.x = x + layout.offset_x, .y = y + layout.offset_y, .width = width, .height = height};
}
...
[[nodiscard]] bool caption_gap_rect(const CaptionLayout& layout, const Rect& caption, Rect& gap) {
  const Rect& bounds = layout.bounds;
...
std::uint32_t background_behind(const lv_obj_t* object) {
  while (object != nullptr) {
    if (lv_obj_get_style_bg_opa(object, LV_PART_MAIN) > LV_OPA_TRANSP) {
      return lv_color_to_u32(lv_obj_get_style_bg_color(object, LV_PART_MAIN)) & 0x00FF'FFFFU;
    }
...
CaptionMask caption_mask_for(const CaptionLayout& layout, const std::int32_t width,
...
bool resolve_frame_box(const Layout& layout, const Config& config, const char* const tag,
                       ...
                       lv_obj_t*& parent, Rect& bounds, std::int32_t& caption_height) {
  const bool has_title = config.title.text.front() != '\0';
  const lv_font_t* const title_font = has_title ? fonts.resolve(config.title.font) : nullptr;
  if (has_title && title_font == nullptr) {
    log::error(tag, "Widget title font is unavailable");
    return false;
  }
  const std::int32_t title_width = has_title ? text_width(title_font, config.title.text.data()) : 0;
  const std::int32_t title_height = has_title ? lv_font_get_line_height(title_font) : 0;
  caption_height = title_height;
  const std::int32_t caption_width = has_title ? title_width + 2 * config.title.gap_padding_px : 0;
  ...
  if (!resolve_widget_bounds(layout, config, config.placement, framed_width + horizontal_insets,
                             framed_height + vertical_insets, fill_available_width, parent,
                             bounds) ||
    ...
    log::error(tag, "Widget needs %dx%d but is placed at %dx%d",
               ...
               static_cast<int>(config.placement.width), static_cast<int>(config.placement.height));
    return false;
  }
  return true;
}

void build_caption(const Config& config, const fonts::Registry& fonts, lv_obj_t* const parent,
                   const Rect& bounds, Box& box) {
  ...
  const std::uint32_t gap_rgb =
      caption_mask_reads_parent(config) ? background_behind(parent) : config.background_color;
  const std::int32_t width = text_width(title_font, config.title.text.data());
  box.caption_mask = caption_mask_for(box.caption_layout, width, gap_rgb & 0x00FF'FFFFU);
  ...
  lv_obj_set_style_text_color(box.caption, lv_color_hex(config.title.color), LV_PART_MAIN);
