...
bool square_fill(const Config& config) {
  return config.fill_corners == configuration::FillCorners::square && config.border.radius_px != 0;
}
...
void apply_background_gradient(const Config& config, lv_obj_t* const object) {
  ...
  lv_obj_set_style_bg_grad_color(object, lv_color_hex(config.background_grad_color), LV_PART_MAIN);
  lv_obj_set_style_bg_grad_dir(
      object,
      config.background_grad_dir == configuration::GradientDirection::horizontal ? LV_GRAD_DIR_HOR
                                                                                 : LV_GRAD_DIR_VER,
      LV_PART_MAIN);
}

void style_container(const Config& config, const Rect& bounds, lv_obj_t* const container) {
  const bool has_background = config.background_color != configuration::kTransparentColor;
  ...
  if (has_background && paints_container) {
    lv_obj_set_style_bg_color(container, lv_color_hex(config.background_color), LV_PART_MAIN);
    apply_background_gradient(config, container);
  }
  lv_obj_set_style_bg_opa(
      container, has_background && paints_container ? LV_OPA_COVER : LV_OPA_TRANSP, LV_PART_MAIN);
  lv_obj_set_style_border_color(container, lv_color_hex(config.border.color), LV_PART_MAIN);
  lv_obj_set_style_border_width(container, config.border.width_px, LV_PART_MAIN);
  lv_obj_set_style_border_opa(container, config.border.width_px == 0 ? LV_OPA_TRANSP : LV_OPA_COVER,
                              LV_PART_MAIN);
...
void style_background_fill(const Config& config, const Rect& bounds, lv_obj_t* const fill) {
  ...
  if (has_background) {
    lv_obj_set_style_bg_color(fill, lv_color_hex(config.background_color), LV_PART_MAIN);
    apply_background_gradient(config, fill);
  }
  lv_obj_set_style_bg_opa(fill, has_background ? LV_OPA_COVER : LV_OPA_TRANSP, LV_PART_MAIN);
...
bool caption_mask_reads_parent(const Config& config) {
  const bool paints_own_fill = config.background_color != configuration::kTransparentColor &&
                               config.background_inset_px == 0;
  return !paints_own_fill;
}

bool bind_source(const std::string_view name, const std::uint8_t modifier_count,
                 const std::span<const configuration::ValueModifier> modifiers,
                 const telemetry::ITelemetryRegistry& registry,
                 const telemetry::ITelemetryReader& telemetry,
                 const ModifierReaders& modifier_readers, value_smoothing::Service* const smoothing,
                 SourceContext& context, ValueReadCallback& read, void*& read_context,
                 bool& fast_updates) {
  ...
  if (modifier_count == 1) {
    const ModifierReader reader = modifier_reader(modifier_readers, modifiers.front().type);
    read = reader.read;
...
bool bind_frame(const configuration::WidgetFrame& frame,
                const telemetry::ITelemetryRegistry& registry,
                const telemetry::ITelemetryReader& telemetry,
                const ModifierReaders& modifier_readers, SourceContext& condition_context,
                SourceContext& caption_context, ValueBinding& binding) {
  bool fast_updates{};
  if (watches_value(frame) &&
      !bind_source(configuration::value_binding_view(frame.condition_source.binding),
                   frame.condition_source.modifier_count, frame.condition_source.modifiers,
                   registry, telemetry, modifier_readers, nullptr, condition_context,
                   binding.condition_read, binding.condition_context, fast_updates)) {
    return false;
  }
  return !binds_caption(frame) ||
         bind_source(configuration::value_binding_view(frame.title.source.binding),
                     frame.title.source.modifier_count, frame.title.source.modifiers, registry,
                     telemetry, modifier_readers, nullptr, caption_context, binding.caption_read,
                     binding.caption_context, fast_updates);
}
...
bool build(const Layout& layout, const Config& config, const char* const tag,
           ...
           const bool fill_available_width, const fonts::Registry& fonts, lv_obj_t*& parent,
           Rect& bounds, Box& box) {
  if (!internal::resolve_frame_box(layout, config, tag, content_width, content_height,
                                   fill_available_width, fonts, parent, bounds,
                                   box.caption_height)) {
    return false;
  }
...
bool update(const Layout& layout, const Config& config, const char* const tag,
            const std::int32_t content_width, const std::int32_t content_height,
            const bool fill_available_width, const fonts::Registry& fonts, Box& box,
            Rect* const resolved_bounds) {
