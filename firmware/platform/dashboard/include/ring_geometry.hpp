...
[[nodiscard]] inline bool inside_sector(const float start_deg, const float sector_deg,
                                        const float degrees) {
...
[[nodiscard]] inline Offset centre_offset(const float start_deg, const float sector_deg,
  ...
  return Offset{(bounds.min_x + bounds.max_x) / 2.0F, (bounds.min_y + bounds.max_y) / 2.0F};
}

}

[[nodiscard]] inline Centre resolve(const Geometry& geometry, const std::int32_t inner_width,
                                    const std::int32_t inner_height) {
  const auto thickness = static_cast<float>(geometry.thickness_px);
  const float fitted =
      static_cast<float>(std::min(inner_width, inner_height)) / 2.0F - thickness / 2.0F;
  const float radius =
      geometry.radius_px != 0 ? static_cast<float>(geometry.radius_px) : std::max(fitted, 0.0F);
  float x = static_cast<float>(inner_width) / 2.0F + static_cast<float>(geometry.x_offset_px);
  float y = static_cast<float>(inner_height) / 2.0F + static_cast<float>(geometry.y_offset_px);
  if (geometry.center_on_figure) {
    ...
    const figure::Offset offset = figure::centre_offset(
        sector_start(geometry.center_angle_deg, sector), sector, radius, thickness);
    x -= offset.x;
    y -= offset.y;
  }
