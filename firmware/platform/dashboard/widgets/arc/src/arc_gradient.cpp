...
[[nodiscard]] RampTable ramp_table(const Ramp& ramp) {
  ...
  for (std::size_t step = 0; step < kRampSteps; ++step) {
    const double ratio = static_cast<double>(step) / static_cast<double>(kRampSteps - 1);
    table[step] = lv_color_hex(fill::color_at(ramp.colours, static_cast<float>(ratio)));
  }
  return table;
}

[[nodiscard]] float fraction_at(const Ramp& ramp, const float dx, const float dy) {
  const float degrees = std::atan2(dy, dx) * 180.0F / std::numbers::pi_v<float>;
  const float travelled = std::fmod(degrees - ramp.start_deg + 720.0F, 360.0F);
  float fraction{};
...
[[nodiscard]] const lv_color_t& ramp_colour(const RampTable& table, const float fraction) {
  const auto step = static_cast<std::size_t>(
      std::clamp(fraction, 0.0F, 1.0F) * static_cast<float>(kRampSteps - 1) + 0.5F);
...
lv_draw_buf_t* prepare(const Ramp& ramp) {
  ...
  if (buffer == nullptr) {
    log::error(kTag, "No memory for a %d px arc gradient", static_cast<int>(ramp.side));
    return nullptr;
  }
  ...
  const float inner = std::max(
      static_cast<float>(ramp.side) / 2.0F - static_cast<float>(ramp.thickness_px) - kEdgeMarginPx,
      0.0F);
  const float outer_squared = outer * outer;
  const float inner_squared = inner * inner;
  const std::size_t pixel_bytes = lv_color_format_get_size(LV_COLOR_FORMAT_NATIVE);
  for (std::int32_t y = 0; y < ramp.side; ++y) {
    std::uint8_t* const row = buffer->data + static_cast<std::size_t>(y) * buffer->header.stride;
