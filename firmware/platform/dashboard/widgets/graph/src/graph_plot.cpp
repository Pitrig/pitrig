...
std::int32_t corner_reserve(const std::uint16_t radius_px, const std::int32_t border_px) {
...
void clear_columns(const State& state, lv_layer_t& layer, const std::int32_t from,
  ...
  if (area.x2 >= state.plot_width) {
    lv_area_t wrapped{area.x1 - state.plot_width, 0, area.x2 - state.plot_width, height - 1};
    lv_draw_rect(&layer, &descriptor, &wrapped);
  }
}

void draw_segment(const State& state, lv_layer_t& layer, const std::uint32_t rgb,
                  const std::int32_t x0, const float y0, const std::int32_t x1, const float y1) {
  ...
  descriptor.p1 = {static_cast<lv_value_precise_t>(x0), static_cast<lv_value_precise_t>(y0)};
  descriptor.p2 = {static_cast<lv_value_precise_t>(x1), static_cast<lv_value_precise_t>(y1)};
