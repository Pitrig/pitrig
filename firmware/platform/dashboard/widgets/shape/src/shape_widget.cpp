...
bool Collection::build(State& state, const Layout& layout, const Config& config,
  ...
  state.painter.configure(config.frame, box, config.frame.border.color, nullptr, nullptr);
...
bool Collection::restyle(const std::size_t index, const Layout& layout, const Config& config,
                         const frame::ValueBinding& binding, const fonts::Registry& fonts) {
  return update_one(index, [&](State& state) {
    frame::Box box{.container = state.container, .background_fill = state.background_fill};
...
bool Collection::create(const Layout& layout, const std::span<const Config> configurations,
                        ...
                        const fonts::Registry& fonts, const std::span<lv_obj_t*> containers) {
  ...
  return build_all(configurations.size(), [&](State& state, const std::size_t index) {
    if (!build(state, layout, configurations[index], bindings[index], fonts)) {
      return false;
    }
