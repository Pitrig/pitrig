...
[[nodiscard]] dashboard::frame::ModifierReaders modifier_readers(
  ...
  readers[static_cast<std::size_t>(configuration::ValueModifierType::lap_timer)] = {
      .read = modules.lap_timer_started ? &read_lap_timer_modifier : nullptr,
      .context = modules.lap_timer_started ? static_cast<void*>(&modules.lap_timer) : nullptr,
  };
  return readers;
}

void wake_widgets(void* const context) { static_cast<Dashboard*>(context)->widgets.wake_all(); }

void on_screen_shown(void* const context) {
  Dashboard& dashboard = *static_cast<Dashboard*>(context);
  dashboard.widgets.wake_all();
  dashboard.slots.wake();
}
...
[[nodiscard]] bool register_widget_types(Dashboard& dashboard) {
  ...
  dashboard.graph.collection.set_renders_when_hidden(true);
  const bool registered =
      dashboard.widgets.add(widget_descriptor<SlotWidgetOps<SlotWidgets>>(dashboard.slot)) &&
      dashboard.widgets.add(widget_descriptor<ValueWidgetOps<ShapeWidgets>>(dashboard.shape)) &&
      dashboard.widgets.add(widget_descriptor<ValueWidgetOps<TextWidgets>>(dashboard.text)) &&
      dashboard.widgets.add(widget_descriptor<ValueWidgetOps<BarWidgets>>(dashboard.bar)) &&
      dashboard.widgets.add(widget_descriptor<ValueWidgetOps<ArcWidgets>>(dashboard.arc)) &&
      dashboard.widgets.add(
          widget_descriptor<ValueWidgetOps<IndicatorWidgets>>(dashboard.indicator)) &&
      dashboard.widgets.add(widget_descriptor<ValueWidgetOps<GraphWidgets>>(dashboard.graph)) &&
      dashboard.widgets.add(widget_descriptor<ValueWidgetOps<ImageWidgets>>(dashboard.image));
  lvgl_port_unlock();
  if (!registered) {
...
[[nodiscard]] bool attach_slots_and_navigation(
    const configuration::ApplicationConfiguration& configuration, const dashboard::Layout& layout,
    Dashboard& dashboard) {
  if (!lvgl_port_lock(0)) {
  ...
  dashboard.navigation.set_screen_shown(&on_screen_shown, &dashboard);
  lvgl_port_unlock();
  return attached;
}
...
bool show_startup_screen(lv_display_t* const display,
                         const configuration::ApplicationConfiguration& configuration) {
  (void)configuration;
  return g_dashboard.splash.show(display, lv_display_get_layer_top(display));
}

void dismiss_startup_screen(const configuration::ApplicationConfiguration& configuration,
                            const bool wait_for_minimum) {
  if (!screens::will_render_content(configuration)) {
    return;
  }
  g_dashboard.splash.dismiss(wait_for_minimum ? kMinimumStartupScreenDurationMs : 0);
}
...
bool create(lv_display_t* const display,
  ...
  const std::size_t screen_count = screens::create(display, configuration, dashboard_state);
  if (screen_count == 0) {
  ...
  if (initialized && !screens::apply_container_clipping(configuration, dashboard_state)) {
...
#if PITRIG_DEBUG
  dashboard::overlays::create(dashboard_state.overlays, display, telemetry_transport);
#endif
  return initialized;
}
...
#if PITRIG_DEBUG
  dashboard::overlays::destroy(dashboard.overlays);
#endif
}
...
bool start_render_trigger(Dashboard& dashboard, events::EventBus& event_bus) {
  ...
  if (!dashboard.render_trigger.start(&wake_widgets, &dashboard, g_render_trigger_task)) {
    log::error(kTag, "Failed to start the render trigger task");
    return false;
  }
  dashboard.telemetry_subscription =
      event_bus.subscribe(telemetry::kTelemetryUpdatedEvent, &on_telemetry_updated, &dashboard);
