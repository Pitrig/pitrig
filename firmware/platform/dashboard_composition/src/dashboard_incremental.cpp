...
using incremental::changed_containers;
...
[[nodiscard]] bool add_screens(const configuration::DashboardConfiguration& before,
  ...
  const std::size_t count = screens::extend(display, next, dashboard, before.screen_count);
  const bool added = count >= next.dashboard.screen_count;
  if (added) {
    const std::span<lv_obj_t* const> screens = std::span{dashboard.screens}.first(count);
...
[[nodiscard]] bool recolour_screens(const configuration::DashboardConfiguration& before,
                                    const configuration::DashboardConfiguration& after,
                                    Dashboard& dashboard, std::uint32_t& recoloured) {
  ...
  for (std::size_t index = 0; index < after.screen_count; ++index) {
    if (before.screens[index].background_color == after.screens[index].background_color) {
      continue;
    }
    if (!lvgl_port_lock(0)) {
      return false;
    }
    lv_obj_set_style_bg_color(dashboard.screens[index],
...
bool apply_incremental(const configuration::ApplicationConfiguration& previous,
  ...
  if (after.screen_count > before.screen_count && !add_screens(before, next, dashboard)) {
  ...
  const incremental::ContainerChanges containers = changed_containers(before, after);
  ...
  for (std::size_t entry = 0; entry < dashboard.widgets.type_count(); ++entry) {
    ...
    const configuration::WidgetTypeTraits& traits = configuration::widget_traits(type);
    ...
    for (std::uint8_t index = 0; index < count; ++index) {
      if (!needs_update(traits, before, after, index, previous_count, recoloured_screens,
                        containers)) {
        continue;
      }
