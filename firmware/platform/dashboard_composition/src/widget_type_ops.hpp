...
struct WidgetOpsCommon {
  static void destroy(void* const context) { storage(context).collection.destroy(); }
  ...
  [[nodiscard]] static bool sync_count(void* const context, const std::uint8_t count) {
 ...
 protected:
  static Storage& storage(void* const context) { return *static_cast<Storage*>(context); }

  static std::span<const typename Storage::Config> configurations(const Storage& widgets) {
    const auto& pool = widgets.dashboard->*Storage::kPool;
    return {pool.data(), configuration::widget_traits(Storage::kType).count(*widgets.dashboard)};
  }

  [[nodiscard]] static bool rebind(Storage& widgets,
                                   const std::span<const typename Storage::Config> configurations) {
    if constexpr (Storage::kSmoothsSource) {
      return widgets.binder.bind(configurations, *widgets.registry, *widgets.telemetry,
                                 widgets.modifier_readers, widgets.smoothing);
    } else {
      return widgets.binder.bind(configurations, *widgets.registry, *widgets.telemetry,
                                 widgets.modifier_readers);
    }
  }
  ...
  [[nodiscard]] static bool report_create_failure() {
    log::error("dashboard", "Failed to create %.*s widgets", static_cast<int>(type_name().size()),
               type_name().data());
    return false;
  }

  static std::string_view type_name() { return configuration::widget_traits(Storage::kType).name; }
};
...
struct ValueWidgetOps : WidgetOpsCommon<Storage> {
  ...
  static void wake(void* const context) { Common::storage(context).collection.wake(); }
  ...
  [[nodiscard]] static bool create(void* const context) {
    ...
    const bool created = [&] {
      if constexpr (requires { widgets.images; }) {
        return widgets.collection.create(widgets.layout, configurations, widgets.binder.bindings(),
  ...
  [[nodiscard]] static bool update_instance(void* const context, const std::uint8_t index) {
    Storage& widgets = Common::storage(context);
    const auto configurations = Common::configurations(widgets);
    if (index >= configurations.size() || !Common::rebind(widgets, configurations)) {
      return false;
    }
    if constexpr (requires { widgets.images; }) {
      return widgets.collection.recreate(index, widgets.layout, configurations[index],
...
struct SlotWidgetOps : WidgetOpsCommon<Storage> {
  ...
  [[nodiscard]] static bool create(void* const context) {
    Storage& widgets = Common::storage(context);
    if (!widgets.collection.create(widgets.layout, Common::configurations(widgets), *widgets.fonts,
                                   widgets.page_slots)) {
  ...
  [[nodiscard]] static bool update_instance(void* const context, const std::uint8_t index) {
    ...
    return widgets.collection.update(index, widgets.layout, configurations[index], *widgets.fonts,
                                     widgets.page_slots);
  }
};
...
[[nodiscard]] dashboard::WidgetDescriptor widget_descriptor(Storage& widgets) {
  dashboard::WidgetDescriptor descriptor{
      .type = Storage::kType,
      .enabled = configuration::widget_traits(Storage::kType).count(*widgets.dashboard) > 0,
      .create = &Ops::create,
      .destroy = &Ops::destroy,
