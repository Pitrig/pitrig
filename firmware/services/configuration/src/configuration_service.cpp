...
bool ConfigurationService::initialize(IConfigurationStorage& storage,
  ...
  if (record_buffer.size() < kRecordBufferSize || payload_buffer.size() < kPayloadBufferSize ||
      configuration_buffer.size() < kConfigurationBufferSize) {
    return false;
  }
  active_ = new (configuration_buffer.data()) ApplicationConfiguration{};
  scratch_ = new (configuration_buffer.data() + sizeof(ApplicationConfiguration))
  ...
  *scratch_ = {};
  for (std::size_t index = 0; index < kConfigurationDocumentCount; ++index) {
    ...
    if (payload.empty() || payload.size() > payloads_[index].size() ||
        !parse_configuration_json(document, payload, validation_profile_, (*scratch_)).ok()) {
      return false;
    }
  ...
  for (std::size_t index = 0; index < kConfigurationDocumentCount; ++index) {
    ...
    remember_payload(document, std::span<const std::uint8_t>(
                                   record_buffer_.data() + kRecordHeaderSize, loaded.payload_size));
  }
  return true;
}
...
ValidationFailure ConfigurationService::validate_payload(
    const ConfigurationDocument document, const std::span<const std::uint8_t> payload) {
  copy_active_to_scratch();
  return parse_configuration_json(document, payload, validation_profile_, (*scratch_));
}
...
ConfigurationService::SaveOutcome ConfigurationService::save(
  ...
  const ValidationFailure parsed =
      parse_configuration_json(document, payload, validation_profile_, (*scratch_));
  if (!parsed.ok()) {
  ...
  remember_payload(document, std::span<const std::uint8_t>(
                                 record_buffer_.data() + kRecordHeaderSize, verified.payload_size));
  return {};
}

bool ConfigurationService::erase(const ConfigurationDocument document) {
  if (storage_ == nullptr || !status_.storage_available || !storage_->erase(document)) {
    return false;
  }
  const std::size_t index = index_of(document);
...
bool ConfigurationService::reset() {
  if (storage_ == nullptr || !status_.storage_available || !storage_->reset()) {
    return false;
  }
  for (std::size_t index = 0; index < kConfigurationDocumentCount; ++index) {
    status_.documents[index] = {};
    remember_payload(static_cast<ConfigurationDocument>(index), factory_payloads_[index]);
  }
  return true;
}

ConfigurationService::LoadedRecord ConfigurationService::load_document(
    const ConfigurationDocument document, ApplicationConfiguration& configuration) {
  LoadedRecord loaded;
  ...
  std::size_t size{};
  const StorageRead stored = storage_->read(document, record_buffer_, size);
  if (stored == StorageRead::absent || (stored == StorageRead::loaded && size == 0)) {
    return loaded;
  }
  loaded.status.outcome = DocumentOutcome::malformed_record;
  if (stored != StorageRead::loaded || size < kRecordHeaderSize) {
    return loaded;
  }
  ...
  if (binary::read_u32_le(record, 0) != kRecordMagic ||
      binary::read_u16_le(record, 4) != kRecordVersion || payload_size == 0 ||
  ...
  const std::span<const std::uint8_t> payload = record.subspan(kRecordHeaderSize, payload_size);
  ...
  const ValidationFailure failure =
      parse_configuration_json(document, payload, validation_profile_, configuration);
  if (!failure.ok()) {
...
bool ConfigurationService::build_record(const std::span<const std::uint8_t> payload,
  ...
  binary::write_u32_le(output, 8, static_cast<std::uint32_t>(payload.size()));
  binary::write_u32_le(output, 12, generation);
  binary::write_u32_le(output, 16, binary::crc32(payload));
  std::copy(payload.begin(), payload.end(), output.begin() + kRecordHeaderSize);
  size = kRecordHeaderSize + payload.size();
  return true;
}

void ConfigurationService::remember_payload(const ConfigurationDocument document,
...
ValidationFailure ConfigurationService::stage(const ConfigurationDocument document,
                                              const std::span<const std::uint8_t> payload) {
  copy_active_to_scratch();
  return parse_configuration_json(document, payload, validation_profile_, (*scratch_));
}

void ConfigurationService::promote() { promote_scratch(); }

void ConfigurationService::revert() { promote_scratch(); }

void ConfigurationService::promote_scratch() { std::swap(active_, scratch_); }

void ConfigurationService::copy_active_to_scratch() { *scratch_ = *active_; }
