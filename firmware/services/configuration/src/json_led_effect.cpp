...
[[nodiscard]] bool parse_gate(const cJSON* const object, LedEffect& config,
  ...
  return read_enum(object, "gate", config.gate, led_gate_from_name, kName, failure) &&
         parse_value_source(object, "condition_source", config.condition_source, kName, failure) &&
         read_array(
...
[[nodiscard]] bool parse_colors(const cJSON* const object, LedEffect& config,
                                ValidationFailure& failure) {
  return read_color(object, "background_color", config.background_color, kName, failure) &&
         read_array(
             object, "color_rules", config.color_rules, config.color_rule_count, kName,
             ValidationError::invalid_module, failure,
             [&](const cJSON* const rule, LedColorRule& parsed) {
               return valid_object(rule, schema::kLedColorRuleKeys, kName, failure) &&
...
[[nodiscard]] bool parse_painting(const cJSON* const object, LedEffect& config,
  ...
  return read_color(object, "color", config.color, kName, failure) &&
         ...
         read_array(
             object, "stops", config.stops, config.stop_count, kName,
             ValidationError::invalid_module, failure,
             ...
             "stops") &&
         read_array(
             object, "steps", config.steps, config.step_count, kName,
             ValidationError::invalid_module, failure,
             [&](const cJSON* const step, IndicatorSegment& parsed) {
               return read_indicator_segment(step, parsed, kName, failure);
             },
             "steps");
}
...
[[nodiscard]] bool parse_content(const cJSON* const object, LedEffect& config,
                                 ValidationFailure& failure) {
  return read_enum(object, "animation", config.animation, led_animation_kind_from_name, kName,
                   failure) &&
         ...
         read_boolean(object, "sprite_loop", config.sprite_loop, kName, failure) &&
         read_text(object, "text", config.text, kName, failure) &&
         read_enum(object, "font", config.font, led_font_from_name, kName, failure);
}

}

bool parse_led_effect(const cJSON* const object, LedEffect& config, ValidationFailure& failure) {
  if (!valid_object(object, schema::kLedEffectKeys, kName, failure) ||
      !read_enum(object, "type", config.type, led_effect_type_from_name, kName, failure) ||
  ...
  return parse_painting(object, config, failure) && parse_content(object, config, failure);
}
