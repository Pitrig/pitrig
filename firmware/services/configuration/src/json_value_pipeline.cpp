...
[[nodiscard]] bool parse_color_ramp(const cJSON* const object, WidgetFrame& config,
  ...
  if (!valid_object(ramp, schema::kColorRampKeys, kName, failure) ||
      !read_enum(ramp, "target", config.color_ramp.target, color_ramp_target_from_name, kName,
                 failure)) {
    return false;
  }
...
[[nodiscard]] bool parse_conditions(const cJSON* const object, WidgetFrame& config,
                                    ValidationFailure& failure) {
  constexpr std::string_view kName = "widget.conditions";
  if (!parse_value_source(object, "condition_source", config.condition_source,
                          "widget.condition_source", failure)) {
    return false;
  }
  ...
  return read_array(
      ...
      ValidationError::malformed, failure, [&](const cJSON* const rule, WidgetCondition& parsed) {
        return valid_object(rule, schema::kWidgetConditionKeys, kName, failure) &&
               ...
               read_color(rule, "background_color", parsed.background_color, kName, failure) &&
               read_color(rule, "border_color", parsed.border_color, kName, failure) &&
...
[[nodiscard]] bool parse_transform(const cJSON* const object, ValueTransform& transform,
                                   ValidationFailure& failure) {
  constexpr std::string_view kName = "widget.text.transform";
  if (!valid_object(object, schema::kValueTransformKeys, kName, failure) ||
      !read_enum(object, "type", transform.type, value_transform_type_from_name, kName, failure) ||
  ...
  if (transform.type == ValueTransformType::number) {
    return read_integer(object, "decimals", transform.number.decimals, kName, failure) &&
           read_float(object, "scale", transform.number.scale, kName, failure) &&
           read_float(object, "offset", transform.number.offset, kName, failure);
  }
...
[[nodiscard]] bool parse_sources(const cJSON* const object, TextWidgetConfiguration& config,
                                 ValidationFailure& failure) {
  ...
  for (int index = 0; index < count; ++index) {
    ...
    if (!valid_object(source, schema::kTextSourceConfigurationKeys, kName, failure) ||
        !read_text(source, "binding", parsed.binding, kName, failure) ||
        !parse_modifiers(source, parsed, kName, failure)) {
      return false;
    }
    if (const cJSON* const transform = member(source, "transform");
        transform != nullptr && !parse_transform(transform, parsed.transform, failure)) {
      return false;
    }
  }
...
[[nodiscard]] bool parse_action(const cJSON* const object, WidgetAction& action,
  ...
  return valid_object(value, schema::kWidgetActionKeys, name, failure) &&
         read_enum(value, "type", action.type, widget_action_type_from_name, name, failure) &&
...
[[nodiscard]] bool parse_frame(const cJSON* const object, WidgetFrame& frame,
  ...
  if (!read_text(object, "id", frame.id, name, failure) ||
      ...
      !read_color(object, "background_color", frame.background_color, name, failure) ||
      !read_color(object, "background_grad_color", frame.background_grad_color, name, failure) ||
      !read_enum(object, "background_grad_dir", frame.background_grad_dir,
                 gradient_direction_from_name, name, failure) ||
      !read_integer(object, "background_inset_px", frame.background_inset_px, name, failure) ||
      !read_enum(object, "fill_corners", frame.fill_corners, fill_corners_from_name, name,
                 failure) ||
  ...
  if (const cJSON* const padding = member(object, "padding"); padding != nullptr) {
    constexpr std::string_view kPaddingName = "widget.padding";
    if (!valid_object(padding, schema::kWidgetInsetsKeys, kPaddingName, failure) ||
        !read_integer(padding, "left", frame.padding.left, kPaddingName, failure) ||
        !read_integer(padding, "top", frame.padding.top, kPaddingName, failure) ||
        !read_integer(padding, "right", frame.padding.right, kPaddingName, failure) ||
        !read_integer(padding, "bottom", frame.padding.bottom, kPaddingName, failure)) {
      return false;
    }
  }
  ...
  if (const cJSON* const title = member(object, "title"); title != nullptr) {
    ...
    if (!valid_object(title, schema::kWidgetTitleStyleKeys, kTitleName, failure) ||
        !read_text(title, "text", frame.title.text, kTitleName, failure) ||
        !parse_value_source(title, "source", frame.title.source, kTitleName, failure) ||
        !parse_optional_font(title, frame.title.font, failure) ||
        !read_color(title, "color", frame.title.color, kTitleName, failure) ||
        !read_enum(title, "alignment", frame.title.alignment, text_alignment_from_name, kTitleName,
                   failure) ||
        !read_integer(title, "offset_x_px", frame.title.offset_x_px, kTitleName, failure) ||
        !read_integer(title, "offset_y_px", frame.title.offset_y_px, kTitleName, failure) ||
        !read_boolean(title, "border_gap", frame.title.border_gap, kTitleName, failure) ||
        !read_integer(title, "gap_padding_px", frame.title.gap_padding_px, kTitleName, failure)) {
      return false;
    }
  }
  ...
  if (const cJSON* const border = member(object, "border"); border != nullptr) {
    ...
    if (!valid_object(border, schema::kWidgetBorderKeys, kBorderName, failure) ||
        !read_color(border, "color", frame.border.color, kBorderName, failure) ||
        !read_integer(border, "width_px", frame.border.width_px, kBorderName, failure) ||
        !read_integer(border, "radius_px", frame.border.radius_px, kBorderName, failure)) {
      return false;
    }
  }
  return true;
...
[[nodiscard]] bool parse_value_source(const cJSON* const object, const char* const key,
                                      ValueSourceConfiguration& config, const std::string_view name,
                                      ValidationFailure& failure) {
  const cJSON* const source = member(object, key);
  if (source == nullptr) {
    return true;
  }
  return valid_object(source, schema::kValueSourceConfigurationKeys, name, failure) &&
         read_text(source, "binding", config.binding, name, failure) &&
         parse_modifiers(source, config, name, failure);
}
