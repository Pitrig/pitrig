...
[[nodiscard]] bool validate_sprite(const HardwareDeviceConfiguration& device,
  ...
  if (const std::string_view out_of_range = schema::range_error(sprite);
      !out_of_range.empty()) {
  ...
  const std::size_t expected = static_cast<std::size_t>(sprite.width) *
                               sprite.height * sprite.frame_count;
  if (pixels.size() != expected) {
    return reject(failure, ValidationError::invalid_led_sprite,
                  "hardware.sprites.pixels");
  }
...
[[nodiscard]] bool validate_device(const ApplicationConfiguration& configuration,
  ...
  if (!profile.led.offers(device.pin) ||
      duplicate_pin(configuration, index, device.pin)) {
    return reject(failure, ValidationError::invalid_led_pin, "hardware.pin");
  }
  if (const std::string_view out_of_range = schema::range_error(device);
      !out_of_range.empty()) {
