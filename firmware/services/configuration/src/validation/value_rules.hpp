...
[[nodiscard]] bool valid_transform(const ValueTransform& transform, telemetry::ValueType type);
[[nodiscard]] bool valid_font(const font_assets::FontSpec& font);

[[nodiscard]] bool on_display(std::int64_t origin_x, std::int64_t origin_y,
                              const WidgetPlacement& placement, std::int32_t display_width,
