...
[[nodiscard]] bool changes_the_device(const std::span<const std::uint8_t> command) {
  constexpr std::array<std::string_view, 3> kWriting{"APPLY:", "SET:", "RESET"};
  for (const std::string_view word : kWriting) {
    if (command.size() >= word.size() && std::equal(word.begin(), word.end(), command.begin())) {
      return true;
    }
  }
  return false;
...
void ConfigurationControl::handle(const std::span<const std::uint8_t> line) {
  const std::span<const std::uint8_t> command = line.subspan(kControlPrefix.size());

  if (command.size() == 4 && std::equal(command.begin(), command.end(), "INFO")) {
    send_info();
    return;
  }

  if (command.size() == 4 && std::equal(command.begin(), command.end(), "DIAG")) {
    send_diagnostics();
    return;
  }
  ...
  if (command.size() >= kGet.size() && std::equal(kGet.begin(), kGet.end(), command.begin())) {
  ...
  if (command.size() >= kApply.size() &&
    ...
    if (!take_document(command.subspan(kApply.size()), true, document, payload)) {
      return;
    }
    ...
    const ValidationFailure failure = apply_handler_(document, payload, apply_context_);
    if (!failure.ok()) {
  ...
  if (command.size() >= kValidate.size() &&
    ...
    if (!take_document(command.subspan(kValidate.size()), true, document, payload)) {
      return;
    }
    const ValidationFailure failure = service_->validate_payload(document, payload);
    if (!failure.ok()) {
      (void)send_error(failure);
      return;
    }
  ...
  if (command.size() >= kSet.size() && std::equal(kSet.begin(), kSet.end(), command.begin())) {
    if (!take_document(command.subspan(kSet.size()), true, document, payload)) {
      return;
    }
    const ConfigurationService::SaveOutcome outcome = service_->save(document, payload);
    ...
    (void)send_document_reply("@PR:OK:SAVED", document,
  ...
  if (command.size() == 5 && std::equal(command.begin(), command.end(), "RESET")) {
    if (service_->reset()) {
      boot_guard::clear_failures();
  ...
  if (command.size() >= kResetOne.size() &&
    ...
    if (!take_document(command.subspan(kResetOne.size()), false, document, payload)) {
      return;
    }
  ...
  if (command.size() == 6 && std::equal(command.begin(), command.end(), "REBOOT")) {
...
bool ConfigurationControl::take_document(const std::span<const std::uint8_t> argument,
                                         const bool expect_payload, ConfigurationDocument& document,
                                         std::span<const std::uint8_t>& payload) {
  const auto separator =
      std::find(argument.begin(), argument.end(), static_cast<std::uint8_t>(':'));
  const auto name_size = static_cast<std::size_t>(separator - argument.begin());
  if (expect_payload == (separator == argument.end())) {
    (void)send_text("@PR:ERR:unknown_document\n");
    return false;
  }
  const std::string_view name(reinterpret_cast<const char*>(argument.data()), name_size);
  if (!configuration_document_from_name(name, document)) {
    (void)send_text("@PR:ERR:unknown_document\n");
    return false;
  }
  payload = expect_payload ? argument.subspan(name_size + 1U) : std::span<const std::uint8_t>{};
  return true;
}

bool ConfigurationControl::write_reply(const std::span<const std::uint8_t> data) {
  transport::ITransport* const link = reply();
  return link != nullptr && link->write(data);
}

bool ConfigurationControl::write_text(transport::ITransport& link, const char* const text) {
  return link.write(std::span<const std::uint8_t>(reinterpret_cast<const std::uint8_t*>(text),
                                                  std::strlen(text)));
}

bool ConfigurationControl::send_text(const char* const text) {
  transport::ITransport* const link = reply();
  return link != nullptr && write_text(*link, text);
}
...
bool ConfigurationControl::send_error(const ValidationFailure& failure) {
  ...
  const int written = std::snprintf(
      ...
      reason.data(), static_cast<int>(failure.screen_index), static_cast<int>(failure.widget_index),
      static_cast<int>(path.size()), path.data());
  return written > 0 && static_cast<std::size_t>(written) < io_buffer_.size() &&
         write_reply(std::span<const std::uint8_t>(io_buffer_.data(), written));
}

bool ConfigurationControl::send_document_reply(const char* const prefix,
  ...
  const int written =
      std::snprintf(reinterpret_cast<char*>(io_buffer_.data()), io_buffer_.size(), "%s:%.*s%s%s\n",
...
bool ConfigurationControl::send_payload(const ConfigurationDocument document,
                                        const std::span<const std::uint8_t> payload) {
  constexpr std::string_view kPayloadPrefix = "@PR:OK:CONFIG:";
  const std::string_view name = configuration_document_name(document);
  const std::size_t prefix_size = kPayloadPrefix.size() + name.size() + 1U;
  ...
  io_buffer_[position++] = '\n';
  return write_reply(std::span<const std::uint8_t>(io_buffer_.data(), position));
}
