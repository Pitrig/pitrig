...
void ConfigurationControl::send_info() {
  ...
  const std::string_view reset_reason = boot_guard::reset_cause_name(health.cause);
  ...
  int written = std::snprintf(
      ...
      static_cast<int>(board.size()), board.data(), esp_app_get_description()->version,
      static_cast<unsigned>(kConfigurationSchemaVersion), status.storage_available ? 1U : 0U,
      health.safe_mode ? 1U : 0U, static_cast<unsigned>(health.consecutive_failures),
      static_cast<int>(reset_reason.size()), reset_reason.data(),
      static_cast<int>(last_phase.size()), last_phase.data());
  for (std::size_t index = 0; written > 0 && index < kConfigurationDocumentCount; ++index) {
    ...
    const std::string_view outcome = document_outcome_name(status.documents[index].outcome);
    const int field = std::snprintf(reinterpret_cast<char*>(io_buffer_.data()) + written,
  ...
  if (written > 0 && static_cast<std::size_t>(written) + 1U < io_buffer_.size()) {
    io_buffer_[static_cast<std::size_t>(written)] = '\n';
    (void)write_reply(
        std::span<const std::uint8_t>(io_buffer_.data(), static_cast<std::size_t>(written) + 1U));
  }
}
