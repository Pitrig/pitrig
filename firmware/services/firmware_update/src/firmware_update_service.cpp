...
[[nodiscard]] std::string_view label_of(const esp_partition_t* const partition) {
  return partition == nullptr ? std::string_view{} : std::string_view{partition->label};
}
...
bool Service::initialize(const configuration::BoardId board) {
  ...
  if (const esp_app_desc_t* const description = esp_app_get_description(); description != nullptr) {
...
UpdateError Service::begin_update(const std::size_t package_size) {
  ...
  if (package_size <= kImageOffset || package_size - kImageOffset > target_->size) {
...
UpdateError Service::consume_header(std::span<const std::uint8_t>& bytes) {
  ...
  const std::size_t wanted = std::min(bytes.size(), header_.size() - update_received_);
...
UpdateError Service::open_target() {
  ...
  const asset_package::Format format{
      .magic = kMagic,
      .version = kFormatVersion,
      .minimum_version = kFormatVersion,
      .manifest_entry_size = kManifestEntrySize,
      .maximum_entries = kManifestEntryCount,
      .data_offset = kImageOffset,
      .storage_size = update_size_,
  };
  asset_package::Header parsed{};
  if (!asset_package::validate_header_fields(format, header, parsed) ||
      parsed.entry_count != kManifestEntryCount || parsed.payload_size != update_size_) {
    return UpdateError::invalid_package;
  }

  const auto manifest =
      header.subspan(asset_package::kManifestOffset, kManifestEntryCount * kManifestEntrySize);
  if (binary::read_u32_le(header, asset_package::kHeaderManifestCrcOffset) !=
          binary::crc32(manifest) ||
      binary::read_u16_le(manifest, 0) != static_cast<std::uint16_t>(board_)) {
    return UpdateError::invalid_package;
  }

  expected_image_crc_ = parsed.payload_crc;
...
UpdateError Service::write_update(const std::span<const std::uint8_t> bytes) {
  ...
  if (const UpdateError error = consume_header(body); error != UpdateError::none) {
    reset_update();
    return error;
  }
  if (body.empty()) {
...
UpdateError Service::commit_update() {
  if (!update_in_progress_ || !handle_open_ || update_received_ != update_size_) {
    return UpdateError::invalid_state;
  }
