...
constexpr std::string_view kBeginPrefix = "@SC:IMAGE:BEGIN:size=";
constexpr std::string_view kInfoCommand = "@SC:IMAGE:INFO";
constexpr std::string_view kClearCommand = "@SC:IMAGE:CLEAR";
...
ImageAssetControl::~ImageAssetControl() { stop(); }

void ImageAssetControl::consume_command_entry(
    void* const context, const std::span<const std::uint8_t> line) {
  static_cast<ImageAssetControl*>(context)->consume_command(line);
}

void ImageAssetControl::consume_entry(void* const context,
                                     const std::span<const std::uint8_t> bytes) {
  static_cast<ImageAssetControl*>(context)->consume(bytes);
}
...
bool ImageAssetControl::initialize(Service& service,
  ...
  claim_ = &claim;
  session_ = {
      .command_prefix = "@SC:IMAGE:",
      .consume_command = &ImageAssetControl::consume_command_entry,
      .consume = &ImageAssetControl::consume_entry,
      .context = this,
  };
  reset_session();
  request_state_.store(RequestState::idle, std::memory_order_relaxed);
  task_ = xTaskCreateStatic(&ImageAssetControl::task_entry,
                            "image_asset_control", task_stack_.size(), this,
                            kTaskPriority, task_stack_.data(), &task_state_);
  if (task_ != nullptr) {
#if SIMCORE_DEBUG
    performance::register_task(performance::TaskMetric::image_asset_control,
                               task_);
#endif
  } else {
    stop();
...
void ImageAssetControl::stop() {
  if (task_ != nullptr) {
#if SIMCORE_DEBUG
    performance::unregister_task(performance::TaskMetric::image_asset_control);
#endif
    vTaskDelete(task_);
    task_ = nullptr;
  }
...
void ImageAssetControl::consume_command(
...
void ImageAssetControl::consume(const std::span<const std::uint8_t> bytes) {
...
void ImageAssetControl::task_entry(void* const context) {
  static_cast<ImageAssetControl*>(context)->process();
}

void ImageAssetControl::process() {
  while (true) {
    ...
    if (request_type_ == RequestType::begin) {
      ...
      release_request();
      (void)send_text("@SC:ERR:IMAGE:unknown_command\n");
    } else if (request_type_ == RequestType::busy) {
      release_request();
      (void)send_text("@SC:ERR:IMAGE:busy\n");
    } else {
...
void ImageAssetControl::handle_clear() {
  const UpdateError error = service_->clear();
  release_request();
  if (error == UpdateError::none) {
    (void)send_text("@SC:OK:IMAGE:CLEARED:reboot_required=1\n");
    return;
  }
...
void ImageAssetControl::handle_info() {
  const Status& status = service_->status();
  release_request();
  int written = std::snprintf(
      response_.data(), response_.size(),
      "@SC:OK:IMAGE:INFO:storage=%u,package=%u,format=%u,images=%u,size=%lu,reboot_required=%u,entries=",
  ...
  // Each entry carries its geometry, which is what lets the configurator tell
  // whether an installed image still suits the dashboard without re-uploading
  // to find out.
  const auto images = service_->image_catalog();
  for (std::size_t index = 0; index < images.size(); ++index) {
    const auto id = image_id_view(images[index].id);
    written = std::snprintf(
        response_.data() + offset, response_.size() - offset, "%s%.*s:%ux%u:%s",
        index == 0 ? "" : ";", static_cast<int>(id.size()), id.data(),
        static_cast<unsigned>(images[index].width),
...
void ImageAssetControl::handle_begin() {
  ...
  release_request();
  if (!send_text("@SC:OK:IMAGE:READY:max_chunk=1024\n")) {
    service_->cancel_update();
    reset_session();
  }
}

void ImageAssetControl::handle_frame() {
  const auto frame =
  ...
  if (type == FrameType::cancel) {
    ...
    release_request();
    reset_session();
    (void)send_text("@SC:OK:IMAGE:CANCELLED\n");
    return;
  }
  if (type == FrameType::commit) {
    ...
    release_request();
    reset_session();
    (void)send_text("@SC:OK:IMAGE:COMMITTED:reboot_required=1\n");
    return;
  }
...
void ImageAssetControl::queue_request(const RequestType type) {
  RequestState expected = RequestState::idle;
  if (!request_state_.compare_exchange_strong(
          expected, RequestState::writing, std::memory_order_acquire,
          std::memory_order_relaxed)) {
...
void ImageAssetControl::release_request() {
  request_state_.store(RequestState::idle, std::memory_order_release);
}

void ImageAssetControl::finish_with_error(const char* const error,
...
bool ImageAssetControl::send_text(const char* const text) {
...
bool ImageAssetControl::send_ack(const std::uint32_t sequence) {
  const int written = std::snprintf(
      response_.data(), response_.size(),
      "@SC:OK:IMAGE:ACK:sequence=%lu,received=%lu\n",
...
void ImageAssetControl::reset_session() {
  package_size_ = 0;
  received_size_ = 0;
  expected_sequence_ = 0;
  overrun_.store(false, std::memory_order_relaxed);
