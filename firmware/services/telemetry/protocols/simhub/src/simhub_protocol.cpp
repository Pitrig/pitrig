...
[[nodiscard]] bool parse_float(const std::span<const char> text, float& value) {
  return transformers::number_transform::parse({text.data(), text.size()}, value);
}

[[nodiscard]] bool parse_boolean(const std::span<const char> text, bool& value) {
  const std::string_view source{text.data(), text.size()};
  if (source == "1" || source == "true" || source == "True" || source == "TRUE") {
    value = true;
    return true;
  }
  if (source == "0" || source == "false" || source == "False" || source == "FALSE") {
    value = false;
    return true;
  }
  return false;
...
[[nodiscard]] std::uint16_t identifier_key(const std::span<const char> identifier) {
  if (identifier.empty() || identifier.size() > 2) {
    return 0;
  }
  const auto first = static_cast<std::uint8_t>(identifier[0]);
  const auto second =
      identifier.size() == 2 ? static_cast<std::uint8_t>(identifier[1]) : std::uint8_t{};
  return static_cast<std::uint16_t>((static_cast<std::uint16_t>(first) << 8U) | second);
}

}

SimHubProtocol::SimHubProtocol(const telemetry::ITelemetryRegistry& registry) {
  initialized_ = true;
  for (std::size_t index = 0; index < handles_.size(); ++index) {
    handles_[index] = registry.resolve(simhub_catalog::kBindings[index].canonical_name);
...
bool SimHubProtocol::initialized() const { return initialized_; }

telemetry::Handle SimHubProtocol::resolve_identifier(const std::span<const char> identifier) const {
  const std::uint16_t key = identifier_key(identifier);
  const auto entry =
      std::lower_bound(simhub_catalog::kLookup.begin(), simhub_catalog::kLookup.end(), key,
...
void SimHubProtocol::consume_line(const std::span<const std::uint8_t> line,
  ...
  const std::span<const char> text(reinterpret_cast<const char*>(line.data()), line.size());
