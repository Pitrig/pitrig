...
[[nodiscard]] bool apply(const Config& config, float value, std::span<char> output);
[[nodiscard]] bool apply(const Config& config, std::string_view value, std::span<char> output);
